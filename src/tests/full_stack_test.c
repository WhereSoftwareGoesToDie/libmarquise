...
{
	f->context =
	    marquise_consumer_new("ipc:///tmp/marquise_full_stack_test", 0.1);
	g_assert(f->context);
...
void one_message(fixture * f, gconstpointer td)
{
	...
	g_assert(marquise_send_int
		 (f->connection, field_buf, value_buf, 1, 10, 20)
		 != -1);
	...
	int bytes =
	    LZ4_decompress_safe(scratch + 8, decompressed, (47 - 8), 512);
	g_assert_cmpint(bytes, ==, 39);
	free(scratch);
	free(decompressed);

	g_assert(zmq_msg_send(&ident, bind_sock, ZMQ_SNDMORE) != -1);
	g_assert(zmq_send(bind_sock, &msg_id, sizeof(msg_id), ZMQ_SNDMORE) !=
		 -1);
...
void many_messages(fixture * f, gconstpointer td)
{
	...
	for (i = 0; i < 8192; i++)
		g_assert(marquise_send_int
			 (f->connection, field_buf, value_buf, 1, 10,
			  20) != -1);
	...
	while (i < 8192) {
		...
		int bytes = LZ4_decompress_safe(scratch + 8, decompressed,
						(received - 8), 3000000);
		g_assert_cmpint(bytes, >, 0);
		DataBurst *burst;
		burst =
		    data_burst__unpack(NULL, bytes, (uint8_t *) decompressed);
		g_assert(burst);

		int j;
		for (j = 0; j < burst->n_frames; j++) {
			i++;
			g_assert_cmpint(burst->frames[j]->value_numeric, ==,
					10);
		...
		g_assert(zmq_msg_send(&ident, bind_sock, ZMQ_SNDMORE) != -1);
		g_assert(zmq_send
			 (bind_sock, &msg_id, sizeof(msg_id),
			  ZMQ_SNDMORE) != -1);
		g_assert(zmq_send(bind_sock, "", 0, 0) != -1);
	}
...
int main(int argc, char **argv)
{
	...
	g_test_add("/full_stack/one_message", fixture, NULL, setup, one_message,
		   teardown);
	g_test_add("/full_stack/many_messages", fixture, NULL, setup,
		   many_messages, teardown);
	return g_test_run();
	return g_test_run();
}
