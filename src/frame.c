...
DataFrame *build_frame(char **tag_fields, char **tag_values, int tag_count,
		       uint64_t timestamp, DataFrame__Type payload)
{
	...
	for (i = 0; i < tag_count; ++i) {
		frame->source[i] =
		    build_frame_tag(tag_fields[i], tag_values[i]);
		if (!frame->source[i]) {
			frame->n_source = i;
			free_frame(frame);
			return NULL;
...
int aggregate_frames(frame * frames, size_t count, uint8_t * burst)
{
	...
	for (i = 0; i < count; i++) {
		...
		int varint_size =
		    encode_varint_uint64(frames[i].length, varint_buf);
